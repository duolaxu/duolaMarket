...
import Dish from "../index/oderPage/mainContent/dish";
import ShopCart from "../index/oderPage/shopCart";
export default function Search(props) {
    ...
    const renderDishData = () => {
        return dishData_1.map((item) => {
            return <Dish setDishDataDetail={setDishDataDetail} isShowDishDetail={isShowDishDetail} setShowDishDetail={setShowDishDetail} setRenderDishData={setRenderDishData} reRenderDishData={reRenderDishData} props={item} setCartChange={setCartChange} />
        })
    }
    ...
    return (<>
        <View style={{ width: "750rpx", height: `${heightrpx}rpx`, display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "center" }}>
            <View style={{ width: "95%", height: "100rpx", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
                ...
                <View onClick={() => { setClickInput(false) }} style={{ height: "80%", width: "10%", display: "flex", justifyContent: "center", alignItems: "center" }}>取消</View>
            </View>
            <View style={{ width: "750rpx", height: "100%", backgroundColor: "rgb(240,240,240)" }}>
                {renderDishData()}
            </View>
            <ShopCart bottomBackColor={"rgb(89,89,89)"} bottomBtnBackColor={"rgb(255,108,55)"} setShowDishDetail={setShowDishDetail} setClickCart={params?.setClickCart} shopCartCounts={params?.shopCartCounts} />
        </View>
    </>)
}